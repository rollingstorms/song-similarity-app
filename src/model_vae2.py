...
class Time_Freq_Autoencoder_Builder:
    ...
    def build(width, height, depth, filters=(16,32,64), latent_dim=128, kernel_size=5):
        ...
        latent_concat = tf.keras.layers.Concatenate(axis=1)([x_time, x_freq])

        latent_concat = Dense(tfp.layers.MultivariateNormalTriL.params_size(latent_dim))(latent_concat)
        latent_concat = tfp.layers.MultivariateNormalTriL(latent_dim, activity_regularizer=tfp.layers.KLDivergenceRegularizer(prior))(latent_concat)
