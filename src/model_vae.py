...
class Time_Freq_Autoencoder_Builder:
    ...
    def build(width, height, depth, filters=(16,32,64), latent_dim=128, kernel_size=5):
        ...
        prior = tfp.distributions.Independent(tfp.distributions.Normal(loc=tf.zeros(latent_dim//2), scale=1),
        ...
        x_time = Dense(tfp.layers.MultivariateNormalTriL.params_size(latent_dim//2))(x_time)
        latent_time = tfp.layers.MultivariateNormalTriL(latent_dim//2, activity_regularizer=tfp.layers.KLDivergenceRegularizer(prior))(x_time)
        ...
        x_freq = Dense(tfp.layers.MultivariateNormalTriL.params_size(latent_dim//2))(x_freq)
        latent_freq = tfp.layers.MultivariateNormalTriL(latent_dim//2, activity_regularizer=tfp.layers.KLDivergenceRegularizer(prior))(x_freq)
